...
Diesen Text einfach in Claude Code einfügen — Claude richtet alles automatisch ein.

---

## Prompt zum Einfügen in Claude:
...
1. `git pull` — holt alle Änderungen vom Kollegen
...
3. Falls lokale Änderungen: cd ~/Map && git add -A && git commit -m "Manual sync: $(date '+%Y-%m-%d %H:%M')" && git push
4. Berichte kurz: wieviele neue Commits vom Kollegen, wieviele eigene gepusht.
...
Stop-Hook: cd ~/Map && git add -A && git diff --cached --quiet || git commit -m "Auto: wiki sync $(date '+%Y-%m-%d %H:%M')" && git push 2>/dev/null || true
...
Das Map-Repo muss lokal vorhanden sein:
```bash
